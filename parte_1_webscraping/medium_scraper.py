...
def scrape_tag(tag, yearstart, monthstart, yearstop, monthstop):
    ...
    path = os.getcwd()
    path = path + "/TAG_SCRAPES/SCRAP_"+str(yearstart)+str(monthstart)+"_"+str(yearstop)+str(monthstop)+"/medium_"+tag+"_"+str(yearstart)+str(monthstart)+"_"+str(yearstop)+str(monthstop)+".csv"
...
#-----------------------------------------------------------------
    ...
    while(current_date < end_date):
        ...
        soup = BeautifulSoup(chrome_driver.page_source, "html.parser")# "html.parser" features='lxml' html5lib
        ...
        if firstPage:
            with open(path, 'a') as f:
                df.to_csv(f, mode="a", header=True, index = False, sep = ';')
            firstPage=False
        #IF NOT FIRSTPAGE -> NO HEADER
        else:
            with open(path, 'a') as f:
                df.to_csv(f, mode="a", header=False, index=False, sep = ';')
